...
RUN mvn --batch-mode dependency:copy -Dartifact=org.mlflow:mlflow-scoring:2.17.2:pom -DoutputDirectory=/opt/java 
RUN mvn --batch-mode dependency:copy -Dartifact=org.mlflow:mlflow-scoring:2.17.2:jar -DoutputDirectory=/opt/java/jars 
RUN cp /opt/java/mlflow-scoring-2.17.2.pom /opt/java/pom.xml
