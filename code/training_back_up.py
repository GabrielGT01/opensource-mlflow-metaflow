...
class Training(FlowSpec, FlowMixin):
    ...
    @step
    def train_fold(self):
        """Train a model as part of the cross-validation process  production and log system metrics."""
        import mlflow
    ...
    def train_model(self):
        """Train the model that will be deployed to production and log system metrics."""
        import mlflow
        import numpy as np
        ...
        stop_logging = False
        metric_thread = Thread(target=log_system_metrics)
        metric_thread.start()
        ...
        with mlflow.start_run(run_id=self.mlflow_run_id):
            ...
            ax.matshow(self.conf_matrix, cmap="Blues")
            plt.title("Confusion Matrix")
            plt.xlabel("Predicted Labels")
            plt.ylabel("True Labels")
            for (i, j), val in np.ndenumerate(self.conf_matrix):
                ax.text(j, i, f"{val}", ha="center", va="center")

            # Save confusion matrix to buffer
            buf = BytesIO()
            ...
            # Create HTML content for Metaflow Card
            html_content = f"""
            <html>
                <head><title>Evaluation Results</title></head>
                <body>
                    <h2>Evaluation Results</h2>
                    <p><strong>Accuracy:</strong> {self.final_accuracy:.2f}</p>
                    <p><strong>Recall:</strong> {self.recall :.2f}</p>
            ...
            current.card.append(html_content)  # Append HTML content to the card
