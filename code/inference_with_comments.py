...
class Model(mlflow.pyfunc.PythonModel):
    ...
    def predict(
            ...
            #a list incase request is more than one
            result = []
            errors = []
            model_sucessful_input = []

            #analyze each model input, such that the become single entity, and we can catch error
             #this also turns each request to a series instead of dataframe
            for index, row in model_input.iterrows():
                #rows only contain the data series, but turn to data frame as requested by the transformer
                try:
                    ...
                    if transformed_payload is not None:
                        ...
                        #append the worked item for retraining, if it went thru 
                        # feature transformer waas succesuflly able to capture the model_input
                        model_sucessful_input.append(transformed_payload[0])
                        
                        #work on the single item
                        predictions = self.model.predict(transformed_payload, verbose=0)
                        
                        #transform the predictions into readable answer
                        processed_output = self.process_output(predictions)
                        
                        #this is to avoid nesting, since they are processed individually 
                        #it will always pick the first
                        results.append(processed_output[0])
                        
                        model_output = result
                        model_input = model_sucessful_input
                        

                    else:
                        errors.append({"index": index, "error": "Transformation failed"})
                except Exception as e:
                logging.exception("Error processing row at index %d: %s", index, e)
                errors.append({"index": index, "error": str(e)})
                        
                        
            #if placed here, the model input and data is capture automatically
            self.capture_to_jsonl(model_input, model_output)
            ...
            if (
                ...
                #if placed here, the model input and data capture is also hinged on the basis of if data capture is True
                self.capture_to_jsonl(model_input, model_output)
    ...
    def process_output(self, output: np.ndarray) -> list:
        ...
        if output is not None:
            ...
            classes = self.target_transformer.named_transformers_[
             ...
             #vectorise allows me to treat each cases of the prediction  array, rather than go through a for loop
            ...
            # output as JSON.
            #result is a list housing json file
            #p,c use the list expansion to access prediction and confidence
            #assign p and c as prediction and confidence
            result = [
